...
import com.iscas.lndicatormonitor.service.PrometheusIndexAnService;
...
@Service
public class PrometheusIndexAnServiceImpl implements PrometheusIndexAnService {
    ...
    public Object getPrometheusIndex(PrometheusIndex prometheusIndex) throws Exception {
        ...
        if (name.equals("CPU")) {
            ...
            String encodedQueryStringRead = URLEncoder.encode("sum(rate(node_disk_read_bytes_total{device=~\"(/dev/)?(mmcblk.p.+|nvme.+|rbd.+|sd.+|vd.+|xvd.+|dm-.+|md.+|dasd.+)\"}[1m])) by (instance)", "UTF-8").replace("+", "%20");

            String encodedQueryStringWrite = URLEncoder.encode("sum(rate(node_disk_written_bytes_total{device=~\"(/dev/)?(mmcblk.p.+|nvme.+|rbd.+|sd.+|vd.+|xvd.+|dm-.+|md.+|dasd.+)\"}[1m])) by (instance)", "UTF-8").replace("+", "%20");

            String encodedQueryStringIOTime = URLEncoder.encode("sum(rate(node_disk_io_time_seconds_total{device=~\"(/dev/)?(mmcblk.p.+|nvme.+|rbd.+|sd.+|vd.+|xvd.+|dm-.+|md.+|dasd.+)\"}[1m])) by (instance)", "UTF-8").replace("+", "%20");
            ...
            encodedQueryString = URLEncoder.encode(
                    "sum(rate(node_network_receive_bytes_total{device!=\"lo\"}[1m])) by (instance) * 8",
                    "UTF-8"
            ).replace("+", "%20");
            ...
            encodedQueryString = URLEncoder.encode(
                    "sum(rate(node_network_transmit_packets_total{device!=\"lo\"}[1m])) by (instance)",
                    "UTF-8"
            ).replace("+", "%20");
        }else if (name.equals("node_receive_packets")) {
            encodedQueryString = URLEncoder.encode(
                    "sum(rate(node_network_receive_packets_total{device!=\"lo\"}[1m])) by (instance)",
                    "UTF-8"
            ).replace("+", "%20");
        }else if(name.equals("node_cpu_sum")){
            encodedQueryString = URLEncoder.encode(
